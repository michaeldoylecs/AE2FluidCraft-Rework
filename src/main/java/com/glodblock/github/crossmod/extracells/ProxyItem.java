...
public class ProxyItem extends FCBaseItem {
    ...
    protected void addMetaReplacement(int srcMeta, Item replacement, int targetMeta) {
        this.replacements.put(srcMeta, new ProxyReplacement(replacement, targetMeta));
    }

    protected void addMetaReplacement(int srcMeta, ProxyReplacement replacement) {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List info, boolean p_77624_4_) {
        info.add("§7With Extra Cells gone, this is what remains...");
     ...
     * Optionally, override the {@link ProxyReplacement#replaceNBT(NBTTagCompound)}
     * method for custom NBT migration.
     */
    protected static class ProxyReplacement {
         ...
         */
        ProxyReplacement(Item replacement, int replacementMeta) {
