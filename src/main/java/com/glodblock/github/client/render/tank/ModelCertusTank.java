...
public class ModelCertusTank extends ModelBase {
    ...
    public void renderFluid(TileEntity tileEntity, double x, double y, double z, RenderBlocks renderer) {
        ...
        if (tileEntity != null
                && ((TileCertusQuartzTank) tileEntity).getTankInfo(ForgeDirection.UNKNOWN)[0].fluid != null) {
            Fluid storedFluid = ((TileCertusQuartzTank) tileEntity).getRenderFluid();
            float scale = ((TileCertusQuartzTank) tileEntity).getRenderScale();
    ...
    public void renderInnerBlock(Block block, int x, int y, int z, RenderBlocks renderer, IBlockAccess world) {
        ...
        boolean tankUp = world.getTileEntity(x, y + 1, z) instanceof TileCertusQuartzTank;
        boolean tankDown = world.getTileEntity(x, y - 1, z) instanceof TileCertusQuartzTank;
        int meta = 0;
