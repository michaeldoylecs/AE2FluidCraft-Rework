...
public class GuiBaseFluidPatternTerminalEx extends GuiFCBaseMonitor {
    ...
    protected void actionPerformed(final GuiButton btn) {
        ...
        if (this.encodeBtn == btn) {
            FluidCraft.proxy.netHandler.sendToServer(new CPacketFluidPatternTermBtns(
                    "PatternTerminalEx.Encode", (isCtrlKeyDown() ? 1 : 0) << 1 | (isShiftKeyDown() ? 1 : 0)));
            ...
            FluidCraft.proxy.netHandler.sendToServer(new CPacketFluidPatternTermBtns(
                    "PatternTerminalEx.Prioritize", container.prioritize ? PRIORITY_DISABLE : PRIORITY_ENABLE));
        } else if (this.invertBtn == btn) {
            FluidCraft.proxy.netHandler.sendToServer(
                    new CPacketFluidPatternTermBtns("PatternTerminalEx.Invert", container.inverted ? "0" : "1"));
        } else if (this.combineDisableBtn == btn || this.combineEnableBtn == btn) {
            FluidCraft.proxy.netHandler.sendToServer(new CPacketFluidPatternTermBtns(
                    "PatternTerminalEx.Combine", this.combineDisableBtn == btn ? "1" : "0"));
    ...
    public void initGui() {
        ...
        this.fluidPrioritizedEnabledBtn =
                new GuiFCImgButton(this.guiLeft + 97, this.guiTop + this.ySize - 143, "FORCE_PRIO", "DO_PRIO");
        ...
        this.fluidPrioritizedDisabledBtn =
                new GuiFCImgButton(this.guiLeft + 97, this.guiTop + this.ySize - 143, "NOT_PRIO", "DONT_PRIO");
        ...
        this.combineEnableBtn =
                new GuiFCImgButton(this.guiLeft + 87, this.guiTop + this.ySize - 143, "FORCE_COMBINE", "DO_COMBINE");
        this.combineEnableBtn.setHalfSize(true);
        this.buttonList.add(this.combineEnableBtn);

        this.combineDisableBtn =
                new GuiFCImgButton(this.guiLeft + 87, this.guiTop + this.ySize - 143, "NOT_COMBINE", "DONT_COMBINE");
